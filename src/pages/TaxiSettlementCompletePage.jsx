...
export default function TaxiSettlementCompletePage() {
        ...
        if (roomData) {
          ...
          setSettlementData({
            ...
            participants: participants.map((p) => ({
              name: p.nickname,
              distance: calculatedTaxiFare[p.nickname]?.distance || 0,
              amount: calculatedTaxiFare[p.nickname]?.amount || 0,
            })),
          });
        }
        setLoading(false);
  ...
  const settlementLink = roomId ? `${window.location.origin}/taxi/settlement/${roomId}` : "";
  ...
  const handleKakaoShare = async () => {
    ...
    const shareTitle = "택시 정산 내역 확인";
    const shareText = "택시 정산 내역을 확인해보세요!";
  ...
  return (
    <MobileLayout>
      <div className="flex flex-col gap-5 items-center p-5 bg-neutral-50 min-h-screen w-full">
        ...
        {/* Participant Details */}
        <div className="bg-white flex flex-col gap-4 h-[277px] items-start p-5 shrink-0 w-full max-w-[350px]">
          ...
          {settlementData.participants.map((participant, index) => (
            ...
            >
              <div className="flex gap-2 h-6 items-center shrink-0 w-[200px]">
                ...
                <p className="font-medium text-xs text-gray-500">{participant.distance} km</p>
              </div>
