...
export default function Step4ShareRoomLinkPage() {
  const navigate = useNavigate();
  const location = useLocation();
  const roomId = location.state?.roomId;
  const [roomLink] = useState(roomId ? `${window.location.origin}/settlement/room/${roomId}/participant-entry` : "");
  ...
  const handleKakaoShare = async () => {
    ...
    const shareTitle = "정산 방 초대";
    const shareText = "정산에 참여해주세요!";
  ...
  const handleNext = () => {
    navigate("/settlement/receipt/step5", {
      state: { roomId }
    });
  };
