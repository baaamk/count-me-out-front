...
export default function SettlementViewPage() {
          ...
          if (user) {
            ...
            try {
              ...
              if (userSnap.exists()) {
                ...
                setUserNickname(userData.nickname || user.displayName || "");
              }
            } catch (err) {
  ...
  useEffect(() => {
    if (isLoggedIn && userNickname && settlementInfo) {
      // 참여자 목록에 사용자 닉네임이 있는지 확인
      if (settlementInfo.participants.includes(userNickname)) {
        navigate(`/settlement/${uuid}/payment/${userNickname}`);
      }
    }
  ...
  const handleConfirm = () => {
    ...
    navigate(`/settlement/${uuid}/payment/${nickname}`);
  };
